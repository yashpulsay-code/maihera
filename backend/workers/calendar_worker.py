...
Follows the same pattern as DecayWorker.
...
# Windows where calendar sync is suppressed (IST hours)
SUPPRESSED_WINDOWS = [
...
def _is_suppressed() -> bool:
    """Return True if current IST time falls in a suppressed window."""
...
class CalendarWorker:
    ...
    Syncs new and updated events to the brain graph.
    Respects suppression windows — no sync during gym or sleep.
    """
    ...
    def _run_sync(self) -> None:
        ...
        async def _do_sync():
            try:
                ...
                total_changes = summary.get('created', 0) + summary.get('updated', 0)
                if total_changes > 0:
                    logger.info(
        ...
        # Run async sync in a new event loop for the scheduler thread
    ...
    def start(self) -> None:
        ...
        self._run_sync()
        logger.info("CalendarWorker: initial sync complete.")
